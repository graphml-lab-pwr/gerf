"""Find graph retrofitting parameters using grid search (link prediction)."""
import argparse
import json
import os
...
def evaluate_retrofitting(
    ...
    for fname in sorted(os.listdir(embedding_dir))[:1]:
        ...
        metrics = evaluate_link_prediction(z=z_star, data=data)
...
def main():
    ...
    output_dir = os.path.join(DATA_DIR, "hps/GERF_lp/", dataset)
