"""Script for evaluation of embeddings in link prediction."""
import argparse
import json
import os
...
def main():
    # Get params
    with open("experiments/configs/link_prediction.yaml", "r") as fin:
    ...
    for method in tqdm(cfg["embedding_methods"], desc="Embedding methods"):
        ...
        for data, z in load_latents(method=method, dataset=dataset):
            all_metrics.append(evaluate_link_prediction(z=z, data=data))

        # Save
        method_fname = method.replace("/", "_")
        metrics_path = os.path.join(
            DATA_DIR,
            f"metrics/link_prediction/{dataset}/{method_fname}.json",
        )
